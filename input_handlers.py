...
If a handler is returned then it will become the active hadnler for future events.
...
class EventHandler(BaseEventHandler):
    """
    refactored event handler, uses the Engine class
    and tcod to handle events as they come in.
    """
...
class AskUserEventHandler(EventHandler):
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[ActionOrHandler]:
        ...
        # This ignores modifier keys, i.e., ctrl-C, etc.
        if event.sym in {
            tcod.event.K_LSHIFT,
            tcod.event.K_RSHIFT,
            tcod.event.K_LCTRL,
...
class LevelUpEventHandler(AskUserEventHandler):
    ...
    def on_render(self, console: Console) -> None:
        ...
        console.print(
            x=x + 1,
            y=5,
            string=f"b) Strength (+1 on attack, from {self.engine.player.fighter.power}",
        )

        console.print(
            x=x + 1,
            y=6,
            string=f"c) Agility (+1 on defense, from {self.engine.player.fighter.defense}",
        )
...
class SelectIndexHandler(AskUserEventHandler):
    ...
    def ev_mousebuttondown(self, event: "tcod.event.MouseButtonDown") -> Optional[ActionOrHandler]:
        """
        Left click to confirm a selection.
        """
...
class AreaRangedAttackHandler(SelectIndexHandler):
    ...
    Any entity within the area is targeted.
    """
...
class GameOverEventHandler(EventHandler):
    ...
    def on_quit(self) -> None:
        ...
        raise exceptions.QuitWithoutSaving()   # do not make new save
...
class HistoryViewer(EventHandler):
    ...
    def ev_keydown(self, event: "tcod.event.KeyDown") -> Optional[MainGameEventHandler]:
        # Some conditional movement foo, here
        if event.sym in CURSOR_Y_KEYS:
            adjust = CURSOR_Y_KEYS[event.sym]
            if adjust < 0 and self.cursor == 0:
                # On the edge, move from top to bottom
                self.cursor = self.log_length - 1
            elif adjust > 0 and self.cursor == self.log_length - 1:
                # Same with bottom to top movement
                self.cursor = 0
            else:
                # Otherwise move while styaing clampt to the bounds
                # of the history log
                self.cursor = max(
