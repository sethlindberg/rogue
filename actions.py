...
class PickupAction(Action):
    """
    Pickup an item and add it to your inventory, if there's room
    """
...
class ItemAction(Action):
    ...
    def target_actor(self) -> Optional[Actor]:
        """Returns the actor at this destination"""
        return self.engine.game_map.get_actor_at_location(*self.target_xy)

    def perform(self) -> None:
        """Invoke the item's ability, this action will be given
        to provide context."""
...
class TakeStairsAction(Action):
    def perform(self) -> None:
        """
        Take the stairs, if one is on the tile.
        """
...
class ActionWithDirection(Action):
    ...
    def blocking_entity(self) -> Optional[Entity]:
        """
        Returns the blocking entity at the destination
        """
        return self.engine.game_map.get_block_at_dest(*self.dest_xy)

    @property
    def target_actor(self) -> Optional[Actor]:
        """Return the actor at this action's designation"""
...
class MeleeAction(ActionWithDirection):
    def perform(self) -> None:
        ...
        if damage > 0:
            ...
            target.fighter.hp -= damage  # I love this
        else:
            self.engine.message_log.add_message(
